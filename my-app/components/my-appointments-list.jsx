  ...
  switch (status) {
    ...
    case "ASSIGNED":
      return {
        ...
        badgeClass:
          "bg-emerald-900/40 border-emerald-500/60 text-emerald-200 shadow-m shadow-emerald-900/40",
        description:
...
export default function MyAppointmentsList({ initialRequests, highlightId }) {
  const [requests, setRequests] = useState(initialRequests || []);
  const [cancellingId, setCancellingId] = useState(null);
  const highlightRef = useRef(null);

  // Scroll to and highlight the newly created booking
  useEffect(() => {
    if (highlightId && highlightRef.current) {
      setTimeout(() => {
        highlightRef.current?.scrollIntoView({ behavior: "smooth", block: "center" });
        toast.success("Welcome! Here's your booking request. Track it in real-time above.", {
          duration: 5000,
        });
      }, 300);
    }
        ...
        return (
          <Card
            ...
            className={`relative overflow-hidden bg-gradient-to-br from-slate-950/80 via-slate-900/70 to-emerald-950/60 border border-emerald-900/40 hover:border-emerald-400/70 shadow-lg shadow-emerald-950/40 backdrop-blur-sm transition-all duration-200 ${
              isHighlighted ? "ring-2 ring-emerald-400 ring-offset-2 ring-offset-background animate-pulse-slow" : ""
            }`}
          >
            ...
            {isHighlighted && (
              <div className="absolute top-4 right-4 z-10">
                <Badge className="bg-emerald-500/80 text-white animate-pulse">
                  ✨ Just Created
                </Badge>
              </div>
            )}
