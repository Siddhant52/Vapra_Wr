...
/**
 * Admin creates a payout for a mechanic
 */
export async function createMechanicPayout(formData) {
  ...
  try {
    const mechanic = await db.user.findUnique({ where: { id: mechanicId } });

    if (!mechanic || mechanic.role !== "MECHANIC") throw new Error("Mechanic not found");
    if (mechanic.credits < credits) throw new Error("Mechanic doesn't have enough credits");
...
export async function getDashboardStats() {
  const isAdmin = await verifyAdmin();
  if (!isAdmin) return { totalPayoutsReceived: 0, chartData: [] };
  ...
  try {
    ...
    for (const payout of payouts) {
      ...
      if (bucket) bucket.total += payout.netAmount;
    }
    ...
    return { totalPayoutsReceived: 0, chartData: [], error: "Database unavailable" };
  }
}
